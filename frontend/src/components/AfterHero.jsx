...
const FeatureCard = React.memo(
  ({ feature, index, animationVariants, transition }) => {
    return (
      <motion.div
        key={index}
        className="py-10 bg-primary bg-opacity-90 px-6 rounded-xl text-white text-center shadow-lg shadow-indigo-900/10 border border-indigo-900/10"
        variants={animationVariants}
        transition={transition}
        whileHover={{ y: -5 }}
      >
        ...
        <h3 className="text-[1rem] uppercase pb-4 font-heading_two font-bold text-link">
          {feature.title}
        </h3>
        <p className="max-w-[450px] mx-auto">{feature.description}</p>
      </motion.div>
...
const AfterHero = () => {
  ...
  return (
    <div className="px-[1rem] ss:px-[6rem] mt-[15vh] font-body_two">
      <div className="image flex justify-center">
        <motion.h1 
          className="text-link py-4 sm:pb-8 text-[1.5rem] font-bold font-heading_two"
      ...
      <motion.div 
        className="contact-us text-center mt-[13vh] text-white bg-primary bg-opacity-90 rounded-xl p-8 shadow-lg shadow-indigo-900/10 border border-indigo-900/10"
      ...
      >
        ...
        <div className="register p-3 mt-4">
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="transition text-black duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-lg bg-link rounded-2xl hover:bg-sky-500 py-[.46rem] transition duration-500 ease-in-out mx-4 px-6 font-medium"
          >
            Contact Us
          </motion.button>
        </div>
